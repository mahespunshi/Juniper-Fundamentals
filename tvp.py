...
code = input("Which software release are you looking for? (Example: 15.1X49-D100, 17.4R2, 17.4R1, 18.4R1 ..etc)")
...
while not search.__contains__(code) or code == '':
    ...
    code = input("Example: 15.1X49-D100, 17.4R2, 17.4R1, 18.4R1..etc Please choose again: ")
...
for version_from_list in list:
    ...
    if (version2 != "17" and version2 != "18" and version2 != "19" and version2 != "20"):
        ...
        if device != "srxsme":
            version_dev = f"junos-install-{device}*-{version_from_list}.*"
        else:
            version_dev = f"junos-{device}-{version_from_list}.*"
...
if totalCount == 0:
    print("Sorry not find, please confirm device and code you are looking for is correct or visit page - https://junipernetworks.sharepoint.com/teams/CSS/jtac-srx/SitePages/Process.aspx")
